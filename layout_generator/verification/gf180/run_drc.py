...
def generate_drc_run_template(drc_dir: str, run_dir: str, run_tables_list: list = []):
    ...
    if len(run_tables_list) < 1:
        all_tables = [
            os.path.basename(f)
            for f in glob.glob(os.path.join(drc_dir, "scr", "*.drc"))
            if "antenna" not in f
    ...
    with open(gen_rule_deck_path, "wb") as wfd:
        for f in all_tables:
            with open(os.path.join(drc_dir, "scr", f), "rb") as fd:
...
def run_parallel_run(
    ...
    if arguments["--antenna"]:
        drc_path = os.path.join(rule_deck_full_path, "scr", "antenna.drc")
        list_rule_deck_files["antenna"] = drc_path

    ## Run Density if required.
    if arguments["--density"]:
        drc_path = os.path.join(rule_deck_full_path, "scr", "density.drc")
